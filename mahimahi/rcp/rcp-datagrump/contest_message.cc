...
ContestMessage::Header::Header( const string & str )
  : sequence_number( get_header_field( 0, str ) ),
    ...
    RCP_pkt_type( get_header_field( 6, str ) ),
    rtt( get_header_field( 7, str ) ),
    RCP_rate( get_header_field( 8, str ) )
{}
...
{
  header.send_timestamp = timestamp_ns();
}
...
string ContestMessage::Header::to_string( void ) const
{
  return put_header_field( sequence_number )
    ...
    + put_header_field( RCP_pkt_type )
    + put_header_field( rtt )
    + put_header_field( RCP_rate );
}
					 ...
					 const uint64_t recv_timestamp )
{
  ...
  switch (header.RCP_pkt_type) {
  case RCP_FIN:
    header.RCP_pkt_type = RCP_FINACK;
    break;
  case RCP_SYN:
    header.RCP_pkt_type = RCP_SYNACK;
    break;
  case RCP_DATA:
    header.RCP_pkt_type = RCP_ACK;
    break;
  default:
    break;
  }
...
ContestMessage::ContestMessage( const uint64_t s_sequence_number,
				const uint64_t s_RCP_pkt_type ,
				const uint64_t s_rtt ,
				const int64_t s_RCP_rate ,
				const std::string & s_payload )
  : header( s_sequence_number, s_RCP_pkt_type, s_rtt, s_RCP_rate ),
    payload( s_payload )
{}
